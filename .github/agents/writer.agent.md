...
Use the `ask-questions` tool for implementation choices and next-step decisions.
Follow the **Explain → Capture** pattern: explain options in conversation, then
call `ask-questions` with concise labels. Batch up to 4 questions in one call.
