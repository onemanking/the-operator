...
| Passive upgrades     | Permanently expands capacity or reduces penalties through maintenance purchases. Agent and skill capacity ceilings are derived from the canonical prompt id lists so the loadout ceiling scales with available prompt ids. | [src/game/data/PromptIds.ts](../src/game/data/PromptIds.ts), [src/game/data/UpgradeData.ts](../src/game/data/UpgradeData.ts), [src/game/scenes/MaintenanceScene.ts](../src/game/scenes/MaintenanceScene.ts)                                                                                                                                                                                                                                                      |
...
- Passive upgrades permanently change the loadout math, such as adding capacity
  or reducing penalties. Agent and skill capacity upgrades scale from the
  canonical prompt id lists so the available ceiling grows with the authored
  prompt roster.
